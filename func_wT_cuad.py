...
#leo de los archivos las columnas correspondientes a u
    T_desvio = flujos_desvios['T_desvio']
...
#para los niveles de 1.5 m, 3 m, 5 m y 7 m
    ...
    for T_d, w_d, cov_d in zip(T_desvio, w_desvio, cov_wT):
...
# Guardar los resultados en la variable 'cuad_inst'
        cuad_inst.append([T_d, w_d, cov_wT_c1, cov_wT_c2, cov_wT_c3, cov_wT_c4])

    cuad_inst_df = pd.DataFrame(cuad_inst, columns=['T_desvio', 'w_desvio', \
    'cov_wT_c1', 'cov_wT_c2', 'cov_wT_c3', 'cov_wT_c4'])

    return cuad_inst_df

def flujo_wT_cuad_nan(flujos_desvios):

    cuad_inst_df = flujos_desvios[['T_desvio', 'w_desvio']].copy()
